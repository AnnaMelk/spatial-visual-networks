...
def CalculateLinksWithWeigths(adjM, distmatrix, alpha):
    if alpha is not None:
        ...
        ##calculate means for all non-zero values!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        avg_lw = np.mean(w_adjM[w_adjM>0])
        instrength=np.sum(w_adjM,axis=0)
    else:
        w_adjM = adjM
...
def TimeIntegrationMatrix(simVars,outdata,params):
    ...
    for s in range(params['steps']):
        ...
        if((nrec==params['N'])):# or (nsus==0)):
            outdata=UpdateOutput(outdata,simVars,s*params['dt'])
            break
#         if((nsus==0)):# or (nsus==0)):
#             outdata=UpdateOutput(outdata,simVars,s*params['dt'])
#             #print("No sucseptible nodes left")
#             break 
        if((nrec+nsus==params['N'])):# or (nsus==0)):
            outdata=UpdateOutput(outdata,simVars,s*params['dt'])
            #rint("No infected nodes left")
            break 
#        if((ninf==0) or (nsus==0)):
        #    if params['output']<0: #save only the last state if output=-1
            #outdata=UpdateOutput(outdata,simVars,s*params['dt'])
            #print("No infected or susceptible nodes left")
#             break    
...
swarm = Swarm(setup=setup,n=n,pos=pos,pos_center=pos_center,phi=phi, dist=d,noise_phi=nphi,noise_pos=npos,w=w,l=0.)
adjacency_matrix,network=swarm.binary_visual_network(return_networkX=True,threshold=0.02)
...
# print(visual_network)
# fig,ax=plt.subplots(1, figsize=(7,7))
# mySwarm.draw_weighted_network(visual_network,fig=fig,ax=ax,ellipse_edgecolor='k')
...
for i in range(10):
    ...
    #ax.plot(time, activated_fraction,color='r')
